...
<body>
	<%
		ArrayList<SellerVO> list = dao.getVerifySeller();
		int count = 0;
				...
				<div id="border_line">
					...
					<table border="1">
						...
						<tr>
							...
							<td><a href="brandForm.jsp?name=<%=vo.getSeller_company()%>"><%=vo.getSeller_company()%></a></td>
							<td><%=vo.getPhone()%></td>
							<td><%=vo.getReg_date()%></td>
							<td><%=vo.getVerify()%></td>
							<td><input type="button" value="승인"
								onclick="location.href='admin_verify_proc.jsp?seller_id=<%=vo.getSeller_id()%>'"></td>
						</tr>
						<%
							}
						%>
